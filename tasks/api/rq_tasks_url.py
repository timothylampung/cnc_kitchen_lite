...
from . import rq_task_view as views
...
urlpatterns = [
    ...
    re_path(r'^queues/delete-jobs/$', views.delete_job, name='rq_delete_job'),
    ...
    re_path(r'^queues/(?P<queue_index>[\d]+)/(?P<job_id>[-\w\.\:\$]+)/requeue/$', views.requeue_job_view, name='rq_requeue_job'),
    re_path(r'^queues/(?P<queue_index>[\d]+)/(?P<job_id>[-\w\.\:\$]+)/enqueue/$', views.enqueue_job, name='rq_enqueue_job'),
]
